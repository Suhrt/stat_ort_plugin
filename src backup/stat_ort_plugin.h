...
// ── Log macros ─────────────────────────────────────────────────────────
// LOGD is info-level (lifecycle, progress). On Android it's
// ANDROID_LOG_INFO — filtered out by default in release log views.
// LOGE is error-level (real failures only).
#ifdef __ANDROID__
