...
FFI_EXPORT VaaniPipeline* vaani_pipeline_init(
    ...
    LOGD("vaani_pipeline_init: Pipeline ready. encoder_threads=%d vocab_size=%d",
...
char* decode_audio_buffer(VaaniPipeline* p, float* samples, int num_samples) {
    ...
    if (!mel || seq_len <= 0) {
        LOGE("decode_audio_buffer: mel extraction failed or empty");
        goto cleanup;
    }
    ...
    while (t < time_steps) {
        ...
        if (run_status != NULL) {
            LOGE("decode_audio_buffer: decoder run failed at t=%d: %s",
        ...
        if (get_status != NULL) {
            LOGE("decode_audio_buffer: GetTensorMutableData failed at t=%d", t);
        ...
        if (token_id == blank_id) {
            // Blank token: always advance by at least 1, or by duration if larger
            t += (duration > 1) ? duration : 1;
            same_frame_count = 0;
        } else {
            if (duration == 0) {
                // Non-blank with duration 0: emit token but stay on frame
                // Guard against infinite spin on same frame
...
float run_silero_vad(struct VaaniPipeline* p, const float* audio, int len) {
    if (!p->vad || !audio || len != 512) {
        LOGE("run_silero_vad: invalid input (vad=%p, audio=%p, len=%d)", p->vad, audio, len);
        return 1.0f;
    }
    ...
    if (!p->vad_state_initialized) {
        ...
        LOGD("run_silero_vad: state initialized");
    }

    int64_t input_shape[] = {1, len};
    OrtValue* input_tensor = NULL;
    OrtStatus* status = g_ort->CreateTensorWithDataAsOrtValue(
            p->memory_info, (void*)audio, len * sizeof(float),
            input_shape, 2, ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT, &input_tensor);
    if (status) {
        LOGE("run_silero_vad: input tensor failed: %s", g_ort->GetErrorMessage(status));
    ...
    if (status) {
        LOGE("run_silero_vad: state tensor failed: %s", g_ort->GetErrorMessage(status));
        g_ort->ReleaseStatus(status);
    ...
    if (status) {
        LOGE("run_silero_vad: sr tensor failed: %s", g_ort->GetErrorMessage(status));
        g_ort->ReleaseStatus(status);
        g_ort->ReleaseValue(input_tensor);
    ...
    if (status == NULL && output_values[0] != NULL && output_values[1] != NULL) {
        ...
        if (s) {
            LOGE("run_silero_vad: failed to get output data: %s", g_ort->GetErrorMessage(s));
            g_ort->ReleaseStatus(s);
        } else {
        ...
        if (s) {
            LOGE("run_silero_vad: failed to get stateN data: %s", g_ort->GetErrorMessage(s));
            g_ort->ReleaseStatus(s);
        } else {
            memcpy(p->vad_state, new_state, sizeof(p->vad_state));
        }

        LOGD("run_silero_vad: prob=%.4f", probability);
    } else {
        if (status) {
            LOGE("run_silero_vad: inference failed: %s", g_ort->GetErrorMessage(status));
            g_ort->ReleaseStatus(status);
        } else {
            LOGE("run_silero_vad: output tensor NULL");
        }
    }
...
void run_speaker_embedding(struct VaaniPipeline* p, const float* audio, int len, float* out_emb) {
    ...
    if (!fbanks || seq_len <= 0) {
        LOGE("run_speaker_embedding: fbank extraction failed");
        memset(out_emb, 0, EMBEDDING_DIM * sizeof(float));
    ...
    if (status == NULL && output_tensor != NULL) {
        ...
        if (s) {
            ...
            if (norm > 1e-6f) {
                ...
                LOGE("run_speaker_embedding: near-zero embedding norm (%.6f), speaker model may be misconfigured", norm);
                memset(out_emb, 0, EMBEDDING_DIM * sizeof(float));
            }
        }
    } else {
