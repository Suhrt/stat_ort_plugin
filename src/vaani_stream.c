...
//   boundaries. This matters a lot for accuracy: the model decodes a whole
//   continuous utterance far better than short VAD-chopped fragments, so we
//   bias toward keeping speech together. 2500 ms suits dictation with
//   thinking pauses (medical prescriptions, legal drafting); drop toward
//   1200–1500 ms only if you need snappier turn-taking in a conversation.
//   The VAD sustain threshold (see VAD_SUSTAIN_THRESHOLD in vaani_internal.h)
//   controls what *counts* as silence during this window — soft/breath/quiet
//   frames stay inside the segment thanks to the low sustain bar.
//
// MIN_SEGMENT_SAMPLES: segments shorter than this are discarded as likely
//   false starts (cough, mic bump, single short syllable).
#define END_OF_UTTERANCE_SILENCE_SAMPLES   40000          // 2500 ms at 16 kHz
#define MIN_SEGMENT_SAMPLES                SAMPLE_RATE    // 1 s

// ── Automatic gain control (AGC) ───────────────────────────────────────────
// Boosts quiet input so VAD reliably detects speech and the speaker model gets
// adequate signal. It only ever *amplifies* (gain >= 1.0): a frame already at
// or above AGC_TARGET_RMS gets gain 1.0, so normal-volume audio is untouched.
// The gain is derived from each frame's RMS, smoothed across frames (fast to
// rise, slow to fall), capped so silence/noise isn't blown up, and held during
// silence so the next speech onset is already boosted. The ASR encoder
// normalises its own features, so AGC mainly improves detection, not the text.
// Set AGC_ENABLED to 0 to compile it out.
#define AGC_ENABLED          1
#define AGC_TARGET_RMS       0.08f   // ~ -22 dBFS target speech level
#define AGC_NOISE_FLOOR_RMS  0.003f  // below this a frame is treated as silence
#define AGC_MAX_GAIN         12.0f   // cap (~ +21 dB) so noise isn't amplified
#define AGC_ATTACK           0.4f    // gain-rise smoothing per 32 ms frame
#define AGC_RELEASE          0.05f   // gain-fall smoothing per 32 ms frame
...
FFI_EXPORT char* vaani_stream_push_chunk(VaaniStreamState* state,
...
#if AGC_ENABLED
    // Automatic gain: bring quiet frames up toward AGC_TARGET_RMS. Derived from
    // this frame's RMS, smoothed across frames, boost-only, and applied in
    // place so VAD / speaker / decode all see the gained audio.
    {
        float sumsq = 0.0f;
        for (int i = 0; i < num_samples; i++) sumsq += write_ptr[i] * write_ptr[i];
        float rms = sqrtf(sumsq / (float)num_samples);

        if (rms > AGC_NOISE_FLOOR_RMS) {
            float desired = AGC_TARGET_RMS / rms;
            if (desired < 1.0f)         desired = 1.0f;          // never attenuate
            if (desired > AGC_MAX_GAIN) desired = AGC_MAX_GAIN;  // don't blow up noise
            float a = (desired > state->agc_gain) ? AGC_ATTACK : AGC_RELEASE;
            state->agc_gain += a * (desired - state->agc_gain);
        }
        // Silence: hold the current gain (don't decay to 1.0) so the next
        // speech onset is already boosted and VAD catches its start.

        if (state->agc_gain > 1.001f) {
            for (int i = 0; i < num_samples; i++) {
                float v = write_ptr[i] * state->agc_gain;
                if (v >  1.0f) v =  1.0f;        // hard clip to [-1, 1]
    ...
    if (!state->pipeline->vad) {
        ...
        vad_prob  = run_silero_vad(state->pipeline, state->vad_state,
                                   state->vad_context, write_ptr, num_samples);
...
s->agc_gain = 1.0f;   // AGC starts at unity (calloc would leave it 0.0)

// VAD recurrent state and context buffer are per-stream fields, already
// zeroed by the calloc above — so a new session never inherits the
// previous one's tail audio, and concurrent streams on one pipeline stay
// independent. (No action needed here; documented for clarity.)
...
FFI_EXPORT void vaani_stream_close(VaaniStreamState* state) {
    ...
    if (state->wav_out_file) {
        // Debug-only WAV header. The multi-byte fields are written in host
        // byte order, so this produces a correct file only on little-endian
        // hosts (Android/iOS/desktop targets are all little-endian). This is
        // a diagnostics aid, not part of the transcription path.
